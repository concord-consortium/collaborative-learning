...
function validateRelatedSummary(value: unknown, file: string, field: string): RelatedSummaryEntry {
  ...
  return {
    ...
    // Optional on the way in, so a corpus without them reads unchanged.
...
function validatePeerComment(value: unknown, file: string, field: string): PeerCommentEntry {
  ...
  for (const [ratingValue, count] of Object.entries(ratings)) {
    // A whole number of people, and at least one: a value nobody chose has no key.
  ...
  return {
    ...
    // An empty string is a real case: a comment can carry only a tag.
 ...
 * summaries are written down. `import` seeds from it almost as it seeds `expectedRenderFailure`
 * from `expectations.json`: a value already in the manifest wins, except an empty list, which is
 * what every entry starts as and so cannot mean a human chose none.
 *
 * It names no corpus: one source directory is imported under whatever corpus name the command
 * gives it, so a name in here would either be unchecked and free to drift, or checked and wrong.
 */
 ...
 * removed: the middle condition that separates "related summaries help" from "the comments in them
 * help".
 */
