...
/** One rated human comment on a related document, as a corpus records it. */
export interface PeerCommentEntry {
  commentId: string;
  commentUid: string;
  content: string;
  tags: string[];
  /** One count per rating value, keyed loosely for the same reason `agreements` is. */
...
function validateRelatedSummary(value: unknown, file: string, field: string): RelatedSummaryEntry {
  ...
  return {
    summary: asString(record.summary, file, `${field}.summary`),
    agreements: validated,
    // Optional on the way in, so a manifest or a corpus written before peer comments existed reads
    // unchanged and the schema version does not have to move.
    peerComments: record.peerComments == null
      ? []
      : asArray(record.peerComments, file, `${field}.peerComments`)
        .map((entry, index) => validatePeerComment(entry, file, `${field}.peerComments[${index}]`))
  };
}

function validatePeerComment(value: unknown, file: string, field: string): PeerCommentEntry {
  const record = asObject(value, file, field);
  ...
  if (Object.keys(counts).length === 0) {
    fail(file, `${field}.ratings`, "must record at least one rating");
  }

  return {
    commentId: asString(record.commentId, file, `${field}.commentId`),
    commentUid: asString(record.commentUid, file, `${field}.commentUid`),
    // An empty string is a real case: a person can post a comment carrying only a tag.
    content: asString(record.content, file, `${field}.content`),
    tags: asArray(record.tags, file, `${field}.tags`)
      .map((tag, index) => asString(tag, file, `${field}.tags[${index}]`)),
    ratings: counts,
    updatedAt: asNonNegativeNumber(record.updatedAt, file, `${field}.updatedAt`)
  };
}

/**
 * A source directory's `related-summaries.json`, keyed by document id.
 *
 * The manifest is generated by `import` and never committed, so this is where a corpus's related
 * summaries can be written down and reviewed. `import` seeds from it almost as it seeds
 * `expectedRenderFailure` from `expectations.json`: a value already in the manifest wins, except an
 * empty list. `expectedRenderFailure` is `null` when unset, but a list is `[]`, which is what every
 * entry starts as and so cannot mean a human chose none.
 *
 * It names no corpus. One source directory is imported under whatever corpus name the command
 * gives it — the tests import `examples/synthetic-corpus` under three different ones — so a corpus
 * name in here would either be unchecked and free to drift, or checked and wrong.
 */
export interface RelatedSummariesFile {
  schemaVersion: number;
  documents: Record<string, RelatedSummaryEntry[]>;
}

export function validateRelatedSummariesFile(value: unknown, file: string): RelatedSummariesFile {
  const record = asObject(value, file, "relatedSummaries");
  checkSchemaVersion(record, file);
  const documents = asObject(record.documents, file, "documents");
  const validated: Record<string, RelatedSummaryEntry[]> = {};
  for (const [docId, entries] of Object.entries(documents)) {
    const field = `documents.${docId}`;
    validated[docId] = asArray(entries, file, field)
      .map((entry, index) => validateRelatedSummary(entry, file, `${field}[${index}]`));
  }
  return { schemaVersion: kSchemaVersion, documents: validated };
}
 ...
 * `none` sends the parts empty. `ai-counts` sends the related summaries with their peer comments
 * removed, which is what production sent before peer comments reached the prompt: it is the middle
 * condition that separates "related summaries help" from "the comments in them help".
 */
export const extrasModes = ["all", "ai-counts", "none"] as const;
