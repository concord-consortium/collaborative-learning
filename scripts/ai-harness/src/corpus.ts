...
/**
 * The source directory's related summaries, keyed by document id.
 *
 * A corpus's related summaries have nowhere else to live: the manifest is generated by `import` and
 * never committed, so without this file the only way to give a fixture one is to hand-edit a
...
export function importCorpus(options: ImportOptions): ImportResult {
  ...
  for (const name of sourceFiles) {
    ...
    entries.push({
      ...
      // `expectedRenderFailure` above: a value already in the manifest wins, because a human put it
      // there on purpose. An empty list is not such a value — it is what every entry starts as and
      // what every corpus imported before this file existed still holds — so the sidecar fills it.
      // (`??` alone would be wrong here for exactly that reason: `[]` is not nullish, so an
      // existing corpus could never be seeded at all.)
