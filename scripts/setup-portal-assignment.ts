...
import {
  kClueOAuthAppId, findOAuthClientId, verifyOAuthClientId, ensureRedirectUri
} from "./lib/portal-oauth.js";
...
async function main() {
  ...
  if (options.withReport || options.withRedirect) {
    clientId = options.oauthClientId
      ...
      : await findOAuthClientId(portal, kClueOAuthAppId, kMaxAdminPages);
  ...
  if (options.withRedirect && clientId) {
    const redirect = await ensureRedirectUri(portal, clientId, urls.redirect, options.dryRun);
