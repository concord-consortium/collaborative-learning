      ...
      if (rectRef.current) {
        ...
        numbering.forEach((behaviorIndex, axisIndex) => {
          rectSelection.select(
            `.dragRect.${classPrefix}-${classPostfixes[axisIndex]}`
          )
          // TODO: Revisit this `ts-expect-error` and see if it can be removed.
          // @ts-expect-error void => Selection
        ...
        (axisBounds) => {
          ...
          if (length != null && axisBounds != null) {
            rectSelection
              ...
              .data(numbering)// data signify lower, middle, upper rectangles
              .join(
                // @ts-expect-error void is not assignable to selection
