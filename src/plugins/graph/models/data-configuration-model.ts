...
export const DataConfigurationModel = types
    ...
    get joinedCaseDataArrays() {
      ...
      self.filteredCases?.forEach((aFilteredCases, index) => {
          aFilteredCases.caseIds.forEach(
            (id: any) => joinedCaseData.push({plotNum: index, caseID: id}));
        }
      );
  ...
  .views(self => (
    {
      get numericValuesForYAxis() {
        ...
        return self.yAttributeIDs.reduce((acc: number[], yAttrID: string) => {
          const values: any = allGraphCaseIds.map((anID: string) => Number(self.dataset?.getValue(anID, yAttrID)));
  ...
  .views(self => (
    {
      getLegendColorForCase(id: string): string {
        ...
        return legendValue == null ? ''
          // TODO: Determine why toString is needed below.
          : legendType === 'categorical' ? self.getLegendColorForCategory(legendValue.toString())
  ...
  .actions(self => ({
    setDataset(dataset: IDataSet | undefined) {
      ...
      if (dataset) {
        self.filteredCases[0] = new FilteredCases({
          source: dataset, filter: self.filterCase as any, // TODO: remove `as any`
    ...
    setAttribute(role: GraphAttrRole, desc?: IAttributeDescriptionSnapshot) {
      ...
      } else if (desc && role === 'rightNumeric') {
    ...
    _addNewFilteredCases() {
      self.dataset && self.filteredCases
        ?.push(new FilteredCases({
          ...
          source: self.dataset, filter: self.filterCase as any, // TODO: remove `as any`
          onSetCaseValues: self.handleSetCaseValues
        }));
    ...
    setY2Attribute(desc: IAttributeDescriptionSnapshot) {
      ...
      if (isNewAttribute) {
        ...
        // TODO: Make sure using 0 when numberOfPlots is undefined makes sense.
        const existingFilteredCases = self.filteredCases?.[self.numberOfPlots ? self.numberOfPlots - 1 : 0];
