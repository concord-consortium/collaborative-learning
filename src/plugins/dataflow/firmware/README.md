...
`spikerbit-clue-v1.hex` is a universal micro:bit hex (V1+V2) authored in MakeCode.
...
- **Source of record:** [`spikerbit-clue-v1.ts`](./spikerbit-clue-v1.ts) — the MakeCode
  program. This directory is excluded from the CLUE tsconfig/eslint because the
  program uses MakeCode-only globals.
- **Compiled artifact:** `spikerbit-clue-v1.hex` — imported by the app as a string
...
3. Switch to the JavaScript view and paste in `spikerbit-clue-v1.ts`.
...
6. Export the **universal hex** and save it as `spikerbit-clue-v1.hex` in this
   directory.
