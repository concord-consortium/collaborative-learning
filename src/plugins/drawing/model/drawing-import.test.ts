...
describe("import drawing", () => {
  ...
  it("preserves group width and height for unversioned snapshots whose groups already have them", () => {
    // Reproduces the bug where exportJson omits the version field. The migrator
    // used to treat ANY unversioned snapshot as v1.0.0 and zero out group
    // width/height — collapsing v1.1.0 groups whose dimensions were already valid.
    const groupData = {
      type: "group",
      x: 0, y: 0,
      width: 60, height: 60,
