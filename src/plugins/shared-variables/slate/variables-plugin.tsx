...
import {
  Editor, CustomElement, VariableElement, IFieldValues,
  IDialogController, EFormat, IRow, IField, Node, useSerializing, registerElement
...
// FIXME: this whole plugin is a mess and needs some love.
...
export const VariableComponent = ({ attributes, children, element }: RenderElementProps) => {
  ...
  const varClasses = classNames(kVariableClass, highlightClass) || undefined;
...
export function withVariables(editor: Editor) {
  ...
  editor.configureElement = (format: string, controller: IDialogController, node?: CustomElement) => {
    ...
    console.log('variable stuff here!!!');
    // if (hasVariable) {
...
// function renderVariable(node: Node, attributes: RenderAttribute, children: ReactNode, options?: IRenderOptions) {
//   //const { data } = node;
...
//   const reference: string = "anId"; // FIXME. Where would the the ID get put in?
...
function getDialogValuesFromNode(editor: Editor, variables: VariableType[], node?: Node) {
  const values: Record<string, string> = {};
  //const highlightedText = editor.value.fragment.text;
  const highlightedText = editor ? "FIXMEEEE" : "";
  //const reference = getReferenceFromNode(node);
  const reference = "a reference"; // FIXME
  if (reference) {
...
// function getNodeVariable(variables: VariableType[], node?: Node) {
