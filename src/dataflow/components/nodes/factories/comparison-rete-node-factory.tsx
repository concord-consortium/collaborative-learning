...
export class ComparisonReteNodeFactory extends Rete.Component {
  ...
  public builder(node: Node) {
    ...
    return node
      .addInput(inp1)
      .addInput(inp2)
      .addControl(new DropdownListControl(this.editor, "comparisonOperator", node, dropdownOptions, true))
      .addControl(new NumControl(this.editor, "preview", node, true))
      .addOutput(out) as any;
  }

  public worker(node: NodeData, inputs: any, outputs: any) {
    const comparisonOperator = node.data.comparisonOperator;
    let result = 0;
    const n1 = inputs.num1.length ? inputs.num1[0] : node.data.num1;
    const n2 = inputs.num2.length ? inputs.num2[0] : node.data.num2;

    const nodeOperationInfo = NodeOperationInfo.find(op => op.name === comparisonOperator);
