...
import { getImageDimensions, storeImage } from "../utilities/image-utils";
...
export const externalUrlImagesHandler: IImageHandler = {
  ...
  store(url: string, db: DB, userId: string) {
    ...
    return new Promise((resolve, reject) => {
      storeImage(db, userId, undefined, url)
        .then(simpleImage => {
          if (simpleImage.imageUrl === placeholderImage) {
            ...
            resolve({ contentUrl: simpleImage.imageUrl, displayUrl: simpleImage.imageData });
          }
        })
        .catch(() => {
  ...
  store(url: string, db: DB, userId: string) {
    return new Promise((resolve, reject) => {
      ...
      db.firebase.getPublicUrlFromStore(storagePath, storageUrl)
        .then(newUrl => {
          if (newUrl) {
            storeImage(db, userId, undefined, newUrl).then(simpleImage => {
