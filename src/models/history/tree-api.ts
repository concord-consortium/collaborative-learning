...
export interface TreeAPI {
    /**
     * This is called before the container starts doing an undo or redo or
     ...
     * passed back to the container when this action is complete. In the CLUE
     * implementation, this is done by the tree-monitor not by the action
     * itself.
     *
     * @param exchangeId the id of this `startApplyingContainerPatches` request.
     * It should be passed back to the container when this action is complete.
     ...
     * DocumentStore or UndoStore when they call startApplyContainerPatches.
     ...
     * startApplyContainerPatches with the tree's response. So these two ids are
     * here to support that scenario.
     */
    startApplyingContainerPatches(historyEntryId: string, exchangeId: string): Promise<void>;

    /**
     * This is called by the container to do an undo or redo or replay history
     * events. This is how the container sends the tree the patches that is
     * should apply. These are patches that the tree previously sent to the
     * container with `addTreePatchRecord`. If the tree did this right, it
     ...
     * addTreePatchRecord.
     *
     * @param exchangeId the container uses this id to identify this specific
     * set of patches. This exchangeId needs to be passed back when the tree
     * records this change back to the container with addTreePatchRecord.
     *
     ...
     * array.
     */
    applyContainerPatches(historyEntryId: string, exchangeId: string, patchesToApply: readonly IJsonPatch[]): Promise<void>;

    /**
     * This is called after the container has applied all of the patches. Before
     * this is called by the container, all trees modified by the patches will
     ...
     *
     * @param exchangeId an id created by the container to track this request.
     * It should be passed back to the container with addTreePatchRecord after
     ...
     *
     */
    finishApplyingContainerPatches(historyEntryId: string, exchangeId: string): Promise<void>;

    /**
     * TODO: need to bring over updated documentation from prototype 
     */
    applySharedModelSnapshotFromContainer(historyEntryId: string, exchangeId: string, snapshot: any): Promise<void>;
}
