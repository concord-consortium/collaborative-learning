  ...
  public start() {
    this.disposer = autorun(() => {
      ...
      documents.byType(ProblemDocument).forEach((document) => {
        ...
        // its author is in. A new document could be added with an out of date
        // group id. Or a user's group can change. In both cases the document
        // should be updated. This tracks the author's membership, not ownership:
        // a problem document stays its author's whatever group they move to.
        document.setAuthorGroupId(groups.groupIdForUser(document.uid));
        ...
        if (user.isTeacher || document.authorGroupId === user.currentGroupId) {
