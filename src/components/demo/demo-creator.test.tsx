...
describe("DemoCreator Component", () => {
  ...
  it("can render", () => {
    ...
    expect(wrapper.find("ul.student-links").first().contains(
      ...
      <a href="?appMode=demo&amp;fakeClass=1&amp;fakeUser=student:1&amp;fakeOffering=1&amp;problem=1.1" target="_blank">student 1</a>
    )).toBe(true);
    wrapper.find("select.classes").simulate("change", {target: {value: 2}});
    expect(wrapper.find("ul.student-links").first().contains(
      // tslint:disable-next-line:max-line-length
      <a href="?appMode=demo&amp;fakeClass=2&amp;fakeUser=student:1&amp;fakeOffering=1&amp;problem=1.1" target="_blank">student 1</a>
    )).toBe(true);
    ...
    expect(wrapper.find("ul.student-links").first().contains(
      // tslint:disable-next-line:max-line-length
      <a href="?appMode=demo&amp;fakeClass=2&amp;fakeUser=student:1&amp;fakeOffering=1&amp;problem=1.2" target="_blank">student 1</a>
    )).toBe(true);
  });
});
