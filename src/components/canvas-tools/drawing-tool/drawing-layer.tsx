...
import { gImageMap, ImageMapEntryType, ImageMapEntry } from "../../../models/image-map";
  ...
  private debouncedUpdateImage = debounce((url: string) => {
            gImageMap.getImage(url)
              .then(image => {
                ...
                if (imageObj && image.displayUrl) {
                  const _imageObj = imageObj as ImageObject;
                  _imageObj.model.url = image.displayUrl;
                }
                // update react state
                this.setState({
                  isLoading: false,
  ...
  private createDrawingObject(data: DrawingObjectDataType) {
    ...
    switch (data.type) {
      ...
      case "image":
        ...
        drawingObject = new ImageObject(assign({}, data, { url: placeholderImage }));
        break;
    }
