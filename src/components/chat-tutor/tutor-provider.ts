...
// a conversation is stamped and partitioned by. The value decides both which conversation the
// turns belong to and, once the trigger reads it off the first message, which backend answers
// them. The vocabulary lives in shared/ because the unit config schema and the trigger reference
// the same list.
import {
