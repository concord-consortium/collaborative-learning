  ...
  constructor(props: any) {
    ...
    stores.unitLoadedPromise.then(() => {
      ...
      this.disposer = onSnapshot(document, snapshot => {
        ...
        // last known tile.
        // includeSectionHeaders/includePlaceholders keep section dividers and their "put content here"
        // placeholders in the export so both survive editing a document template through the authoring
        // iframe (no-op for section content, which has neither).
        const json = document.content?.exportAsJson({
          includeTileIds: true, includeSectionHeaders: true, includePlaceholders: true
        });
        if (json) {
