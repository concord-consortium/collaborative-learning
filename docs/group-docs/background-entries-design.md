...
A tile is expected to use either `producerId: null` (single producer) or string producerIds (decomposed producers) consistently. The framework treats each `(producerTileId, producerId)` as an independent lock, so a `null` lock and a string lock for the same tile coexist as separate runners with no interaction between them — mixing is well-defined but not recommended.
...
Pure-read clients only observe live producer output when some other client holds the runner lock and is producing. With no writer client present, there's no producer, so the read-only view shows the last saved state — matching existing dataflow read-only behavior outside group docs.
...
**How the right client gets the sensor lock** is a tile-design problem the framework alone can't solve — there's no generic way for the lock layer to tell which client has the hardware. The expected direction is per-sensor-node ownership: the dataflow tile UI lets a user claim a sensor node ("I have this sensor"), and only the claiming client attempts to acquire the corresponding lock. The `producerId` would be the sensor node's id, so each sensor has its own lock and different students can own different sensors on the same tile. Specifics are out of scope for this spec — the framework provides per-producerId lock granularity to support whatever ownership scheme dataflow adopts.
...
The framework itself doesn't require any CLUE UI changes for correctness. Background entries flow through the existing scrubber, undo, and history infrastructure; the framework-level deltas (undo-exclusion, asymmetric conflict resolution, runner-lock acquisition) are handled at the model and storage layers. Several UI changes are nonetheless needed for the feature to be usable in classroom conditions:

| Concern | Required for correctness? | Required for usability? |
|---|---|---|
| Scrubber skip-by-default (see below) | No | Yes — cluttered slider once dataflow runs |
| Sensor-node claim UX (in the dataflow tile) | No (single-sensor case works) | **Yes for the multi-sensor / split-producer case** — see § Producer authority |
| Runner-lock indicator ("who's running") | No | Yes (multi-client sessions are confusing otherwise) |
| "Dataflow is running" indicator | No | Helpful, especially when the runner is on a different client |

A strict-correctness first pass needs zero CLUE UI changes. A deployable first pass for solo or single-runner dataflow needs scrubber filtering. Multi-sensor or split-producer deployments additionally need the sensor claim UX.

### History scrubber

**Until Phase 6 ships, background entries appear in the scrubber as ordinary slider stops.** The framework is functionally correct without any scrubber changes — applying patches, navigating between entries, and respecting `reverted: true` all work the same way. The behaviors below are usability improvements that prevent the scrubber from being overwhelmed by tick entries once dataflow runs for any meaningful time.
