...
The history events are downloaded only when needed for replaying the history. This is done by a `<LoadDocumentHistory>` component. This component uses hooks to load in the history and then update the document with this history. It is kind of strange for a React component to be managing the MST model. But this temporary history MST model is being created by the component only as it is needed, it isn't something shared by multiple components.

The history loading currently doesn't do any batching/paging during the load, so if the history gets large enough this might cause problems. There is a FIXME in the code for this. The component is currently monitoring Firestore collection, so if new history events are added they will be shown immediately without having to close and open the history UI. However this behavior is flakey right now. When new data shows up the hook in `LoadDocumentHistory` causes a re-render which then completely replaces the history in the TreeManager with the new data. In other words the entries aren't really loaded incrementally. We'll probably need to improve this to handle documents with large histories. However this is not the current source of flakiness. The flakiness comes from the tracking of the `currentHistoryEntryIndex`. This property is intended to reflect the last history entry that was "played" into the document associated with the TreeManager. Right now the document is created by copying the main document usually coming from firebase and then asynchronously the history entries are loaded. The current code doesn't have a way to know what history entry actually matches the state of the document that was copied. So the `currentHistoryEntryIndex` can get out of sync with the document content.
