...
| `owner` (who the document belongs to) | creation: kind-declared `ownerType` → owner `uid` (`getDocumentOwner`) plus a group owner's stored `groupId` (`getDocumentOwnerFields`); read: `hasGroupOwner(doc)` and `hasClassOwner(doc)`, each over the uid prefix its minter used | in progress | CLUE-550 Stage 2 (creation-side owner derivation registry-declared for all kinds); CLUE-610 (both guards; `groupId` moved onto this axis, off the container, and then off the owner *question* — the `uid` is the sole authority). CLUE-610 also pinned the owner at creation in firestore.rules: a new document must be owned by the caller, by their own class (`class_<class_hash>`, corroborated by the token), or by a group whose id agrees with the document's own `offeringId` and `groupId`. That last one is agreement, not membership — group membership lives in the Realtime Database and the token carries no group claim, so a student can still create a document owned by another group in their own offering. Closing that needs a group claim in the portal-minted token or membership mirrored into Firestore, and is why this axis is not done. A parallel residual sits on the class side: the token corroborates that the caller belongs to the class named in `class_<class_hash>`, but not that the caller is entitled to mint a document under that owner — any class member can create any number of `class_<class_hash>`-owned, `concurrent: true` documents, each sectioned under "Whole Class" in Sort Work with the whole class granted read and write on its history. That is narrower than the rule this branch replaced (which admitted any `uid` at all), and it is inherent to the design — convergence on one class document requires that any class member can mint it — so it is accepted rather than tracked as work to close. Still to come: the user level, and a getter that returns which owner a document has rather than testing for one |
