...
| `kind` (preset/cohort tag: defaults, presentation, templates) | stored per-doc tag; dereferenced only in the kind registry | done | CLUE-550 Stage 1 (stored + registry seeded); presentation wired by CLUE-610 (no consumer branches on kind). Every kind is written in code; a unit config declares `variant`s of the `classWide` kind rather than kinds of its own — see "Kinds and variants" in [axes.md](./axes.md). A class-wide document's authored title and variant are stamped at creation, so the registry has no read-time consumer at all |
...
| `curriculum` (what the document is about: nothing → unit → investigation → problem) | creation: fixed by the kind's `containerType`, since every container above the class is identified by a curriculum coordinate; read: `getCurriculumLabel(doc)` and `isAboutUnitOnly(doc)` | in progress | CLUE-550 Stage 2 (creation side, every kind); CLUE-610 (the label, and the unit level states its absent fields explicitly so it is queryable); CLUE-610 stored titles added the first yes/no guard, `isAboutUnitOnly`, which titling uses to recognize a name authored at unit scope |
