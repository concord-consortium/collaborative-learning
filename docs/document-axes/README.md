...
| `kind` (preset/cohort tag: defaults, presentation, templates) | stored per-doc tag; dereferenced only in the kind registry | done | CLUE-550 Stage 1 (stored + registry seeded); presentation wired by CLUE-610 (no consumer branches on kind); CLUE-610 scopes a unit-declared kind's definition to its unit behind `getKindDefinitionFor(doc)` — see "Static and dynamic kinds" in [axes.md](./axes.md). A class-wide document's authored title is stamped at creation rather than resolved from its kind, which leaves the registry with no read-time consumer at all |
