...
| `owner` (who the document belongs to) | creation: kind-declared `ownerType` → owner `uid` (`getDocumentOwner`) plus a group owner's stored `groupId` (`getDocumentOwnerFields`); read: `hasGroupOwner(doc)` and `hasClassOwner(doc)`, each over the uid prefix its minter used; pinned at creation by `firestore.rules` | in progress | CLUE-550 Stage 2 (creation-side owner derivation registry-declared for all kinds); CLUE-610 (both guards; `groupId` moved onto this axis, off the container, and then off the owner *question* — the `uid` is the sole authority; the create rules pin the owner). In progress because the rules cannot corroborate group membership — see "What the rules enforce" in [axes-current-state.md](./axes-current-state.md), which also records the accepted class-side residual. Still to come: the user level, and a getter that returns which owner a document has rather than testing for one |
...
group container level (see [axes-current-state.md](./axes-current-state.md)). Canonical slots follow:
