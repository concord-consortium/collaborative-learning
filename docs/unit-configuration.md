...
`classWideDocuments`: (array | undefined) Class-wide collaborative documents for the unit. Each entry is `{ variant, title }` and becomes one auto-created, concurrently-edited document shared by the whole class, one per class per unit (e.g. a Driving Question Board). `variant` is a label so multiple class-wide documents can be added; it has to be unique among this unit's class-wide documents, and must be a camelCase identifier — a lowercase letter followed by letters or digits, with no spaces, hyphens, or other special characters (e.g. `drivingQuestionBoard`). Entries with an invalid or duplicate `variant` are ignored. `title` is the document title, stamped onto each document when it is created, so changing it here renames only documents created afterwards. Units that omit this array create no class-wide documents.
