...
context('Dataflow Tool Tile', function () {
  ...
  describe("Dataflow Tool", () => {
    ...
    describe("Record Data", () => {
      ...
      it("verify deleting dataflow tile", () => {
        clueCanvas.deleteTile("dataflow");
        dataflowToolTile.getDataflowTile().should('not.exist');
      });
    });
    describe("Link table with Recorded Data", () => {
      it("create a small program, select sampling rate and record data", () => {
        const nodes = [ "timer", "demo-output" ];
        clueCanvas.addTile("dataflow");
        dataflowToolTile.getDataflowTile().should("exist");
        dataflowToolTile.getCreateNodeButton(nodes[0]).click();
        ...
        dataflowToolTile.getNodeOutput().eq(0).click();
        dataflowToolTile.getNodeInput().eq(0).click();
        dataflowToolTile.selectSamplingRate("1000");
        dataflowToolTile.getRecordButton().click();
        dataflowToolTile.getCountdownTimer().should("contain", "000:05")
        dataflowToolTile.getStopButton().click();
      });
      it("verify link table from dataflow with recorded data", () => {
        const table = "Table 1"
        clueCanvas.addTile("table");
        cy.linkTableToDataflow("Program 1", "Table 1");
        tableTile.getTableTitle().should("have.text", table);
        tableTile.getWorkspaceColumnHeader().should("have.length", 3);
        tableTile.getWorkspaceColumnHeader().eq(0).should("have.text", "Time (sec)");
        tableTile.getWorkspaceColumnHeader().eq(1).should("have.text", "Timer 1");
        tableTile.getWorkspaceColumnHeader().eq(2).should("have.text", "Demo Output 2");
        tableTile.getTableRow().should("have.length.least", 5)
        tableTile.getTableCellWithRowColIndex(0, 2).should("have.text", "0");
        tableTile.getTableCellWithRowColIndex(1, 2).should("have.text", "1");
        tableTile.getTableCellWithRowColIndex(2, 2).should("have.text", "2");
        tableTile.getTableCellWithRowColIndex(3, 2).should("have.text", "3");
        tableTile.getTableCellWithRowColIndex(4, 2).should("have.text", "4");
      });
      it("verify linked table after clearing the recording", () => {
        const table = "Table 1"
        dataflowToolTile.getRecordingClearButton().click();
        dataflowToolTile.getClearDataWarningClear().click();
        tableTile.getTableTitle().should("have.text", table);
        tableTile.getWorkspaceColumnHeader().should("have.length", 2);
        tableTile.getWorkspaceColumnHeader().eq(0).should("have.text", "x");
        tableTile.getWorkspaceColumnHeader().eq(1).should("have.text", "y");
        tableTile.getTableRow().should("have.length", 1)
        tableTile.getTableCellWithRowColIndex(0, 2).should("have.text", "");
        tableTile.getTableCellWithRowColIndex(0, 3).should("have.text", "");
      });
      it("verify linked table gets dynamically populated", () => {
        const table = "Table 1"
        dataflowToolTile.getRecordButton().click();
        dataflowToolTile.getCountdownTimer().should("contain", "000:03")
        dataflowToolTile.getStopButton().click();
        tableTile.getTableTitle().should("have.text", table);
        tableTile.getWorkspaceColumnHeader().should("have.length", 3);
        tableTile.getWorkspaceColumnHeader().eq(0).should("have.text", "Time (sec)");
        tableTile.getWorkspaceColumnHeader().eq(1).should("have.text", "Timer 1");
        tableTile.getWorkspaceColumnHeader().eq(2).should("have.text", "Demo Output 2");
        tableTile.getTableRow().should("have.length.least", 3)
        tableTile.getTableCellWithRowColIndex(0, 2).should("have.text", "0");
        tableTile.getTableCellWithRowColIndex(1, 2).should("have.text", "1");
        tableTile.getTableCellWithRowColIndex(2, 2).should("have.text", "2");
      });
      it("verify unlink table from dataflow with recorded data", () => {
        const table = "Table 1"
      ...
      it("verify link table again after unlinking once with recorded data", () => {
        const table = "Table 1"
        cy.linkTableToDataflow("Program 1", "Table 1");
        tableTile.getTableTitle().should("have.text", table);
        tableTile.getWorkspaceColumnHeader().should("have.length", 3);
    ...
    describe("Reload clue in different states of Record/Play/Pause/Stop buttons", () => {
      it("verify page reload cause recording to stop and data doesn't clear", () => {
        const table = "Table 1"
        dataflowToolTile.getRecordButton().click();
        dataflowToolTile.getCountdownTimer().should("contain", "000:03");
        cy.reload();
      ...
      it("verify page reload causes play button to reset and data doesn't clear", () => {
        const table = "Table 1"
        dataflowToolTile.getPlayButton().click();
        cy.reload();
      ...
      it("verify page reload causes pause button to reset and data doesn't clear", () => {
        const table = "Table 1"
        dataflowToolTile.getPlayButton().click();
        cy.wait(1000);
        dataflowToolTile.getPauseButton().click();
      ...
      it("verify page reload without confirming clear doesn't clear data", () => {
        const table = "Table 1"
        dataflowToolTile.getRecordingClearButton().click();
        cy.reload();
